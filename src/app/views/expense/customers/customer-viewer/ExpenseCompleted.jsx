...
const ExpenseCompleted = () => {
  ...
  const curr_year = moment(curr_date).format("YYYY");
     ...
     const handleReset = () => {
    setfrom_date_exc(new Date())
    // console.log("sup",from_date_q)
    
  
    localStorage.removeItem("from_date_exc");
      
    localStorage.removeItem("dataKeyState_exc")
    setexpenseList(
      resetuserList.filter(item => item.is_paid == 2 && new Date(moment(item.paid_date).format('YYYY')).getTime() == new Date(curr_year).getTime() ));
    setisAlive(false)
  ...
  (obj) =>
    new Date(moment(obj.paid_date).format('YYYY')).getTime() == new Date(from_date_exc).getTime() 
...
const handleDateChange_Q = (date) => {
  ...
  useEffect(() => {
    if(localStorage.getItem("from_date_exc")){
      setfrom_date_exc(localStorage.getItem("from_date_exc"));
  
    }
    if(localStorage.getItem("dataKeyState_exc")){
      setexpenseList(JSON.parse(localStorage.getItem("dataKeyState_exc")));
      setLoad(false);
    }else{
