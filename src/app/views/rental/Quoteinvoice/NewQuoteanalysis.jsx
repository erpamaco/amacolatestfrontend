...
// import FormDialog from "../equipment/equipmentprice";
import MemberEditorDialog from "../Others/rentalprice";
import MemberEditorDialogcontact from "../../party/partycontact";

// import FormDialog_equipment from "../../views/equipment/Addequipment_popup"
import MemberEquipmentDialog from "./MemberEquipmentDialog";
...
const InvoiceEditor = ({ isNewInvoice, toggleInvoiceEditor }) => {
  ...
  const [terms1, setterms1] = useState('Single shift (260hrs.): Rental will be charged on minimum 10 hours/day, 6 days/week and 26 days/month. Working hours less than 10 hours per day will be considered as full 10 hours. Excess Hours and Fridays shall be calculated on prorata basis.')
  const [terms2, setterms2] = useState('Equipment/Crane/Vehicle will be mobilized only after the receipt of your official Purchase Order with referring our quotation number.')
  const [terms3, setterms3] = useState('Time sheet will start once the Equipment/Crane/Vehicle reaches to your site & time sheet must submit on or before 5th of Every month.')
  const [terms4, setterms4] = useState('Payment Terms: Payment within 30 days from Invoice Cut-off Date')
  const [terms5, setterms5] = useState('Duration of Hire: 1 MONTH/ Extendable')
  const [terms6, setterms6] = useState('No reduction in agreed minimum hire on machine idle due to whether/site condition or during Ramadhan month Etc.')
  const [terms7, setterms7] = useState('Quotation Validity: 6 days from date of offer, after which time it will be subject to review.')
  const [terms8, setterms8] = useState('At anytime AMACO feels that the machine is endangered through misuse, abuse, being used beyond its capacity or in a manner harmful for the same of the machine or delay In Payment/non-payment of dues as per the quotation/contract,AMACO reserves the right to take the equipment back from the site with prior notification.')
  const [terms9, setterms9] = useState('Any downtime incurred due to inspection department failing to inspect duly notified Equipment/Crane/Vehicle on time will not be the responsibility of AMACO.')
  const [terms10, setterms10] = useState('Our above offer is subject to availability to Equipment/Crane/Vehicle, until placement of firm order by the client and confirmation of acceptance by us.')
  const [terms11, setterms11] = useState('Above Rates are not inclusive of VAT and shall be applicable as per the Laws of Kingdom of Saudi Arabia.')
  ...
  const [shouldOpenEditorDialogequipment, setshouldOpenEditorDialogequipment] = useState(false);
  ...
  const [
    shouldOpenConfirmationDialogequipment,
    setshouldOpenConfirmationDialogequipment,
  ] = useState(false);
  ...
  const controlKeyPress = (e, id, nextid, prev, invoiceItemList,dropdown) => {
    ...
    if (e?.keyCode == 39) {
      if (nextid?.includes('equipment_id')) {
        // if (false) {
      ...
      if (--i >= 0) {
        ...
        } else if (r.includes('equipment_id')) {
          // } else if (false) {
          inputRef[parseInt(r)].focus();
        } else {
  ...
  const handleSubmit = (e,s) => {
    ...
    formData.append('terms6', terms6)
    formData.append('terms7', terms7)
    formData.append('terms8', terms8)
    formData.append('terms9', terms9)
    formData.append('terms10', terms10)
    formData.append('terms11', terms11)
    ...
    url.post('rental-sale-quotation', formData)
      .then(function (response) {
        ...
        Swal.fire({
          ...
          .then((result) => {
            if (s == "New") {
              routerHistory.push(navigatePath + "/rental/quote/" + response.data + "/New/1")

            }
            else {
              routerHistory.push(navigatePath + "/rental/quoateview/4")
            }
          })
      })
  ...
  function cancelform() {
    let mode = "full"
    updateSidebarMode({ mode })
    routerHistory.push(navigatePath + "/rental/quoateview/0")
  }
  ...
  const handleDialogClose = () => {
    ...
    url.get("rental_equipment/" + catid).then(({ data }) => {
      let tempItemList = [...state.item];

      setequipmentList1(data.prices)
      tempItemList.map((element, i) => {
        let sum = 0;

        if (indexvalue === i) {

          element['equipment_id'] = catid;
          // element['descriptionss'] = data.equipment[0].description;





          element.equipment_price_list.splice(id, element.equipment_price_list.length);

          data.prices.map((v, i) => {

            element.equipment_price_list.push({
              price: v.price,
    ...
    setshouldOpenEditorDialogequipment(false);
    url.get("rental_equipment").then(({ data }) => {
      console.log("jakk",data)
  ...
  useEffect(() => {
    ...
    url.get(`rentalmjrQuoteInc/${localStorage.getItem('division')}`).then(({ data }) => {
      console.log("rentalmjrQuoteInc",data)
      setData(data?.uom);
      ...
      setusers(data?.users)
      setproList(data?.equipments?.filter(obj => obj.div_id == localStorage.getItem('division')))
    })
  ...
  const equipment_popup = () => {
    setshouldOpenConfirmationDialogequipment(true)
  }
  const setequipmentdescription = (event, index, newValue) => {
    console.log("hello");
    console.log("newValue",newValue)
    if (newValue !== false) {
      url.get("rental_equipment/" + newValue?.id).then(({ data }) => {
        ...
        tempItemList.map((element, i) => {
          ...
          if (index === i) {
            ...
            element['equipment_id'] = newValue?.id;
            element['item_name'] = newValue?.equipment;
            element['descriptionss'] = data.equipment[0].description;

            if (element.equipment_price_list.length >= 1) {
      ...
      setshouldOpenEditorDialogequipment(true)
    }
  }
  // toggle 
  ...
  const setequipmentids = (id, index) => {
    setcatid(id)
    setindexvalue(index)
    setShouldOpenEditorDialog(true)
  }
...
<TableCell className="pl-0 capitalize" align="left" style={{ width: '150px' }}>
  {/* <TextValidator
    label="Item"
    onChange={(event) => setequipmentdescription(event, index)}
    type="text"
    name="equipment_id"
    fullWidth
    variant="outlined"
    ...
    inputProps={{
      ref: setRef(index + 'equipment_id'),

    }}
    onKeyDown={(e) => { controlKeyPress(e, index + 'equipment_id', index + 'description', null, invoiceItemList) }}

    size="small"
    value={item.equipment_id ? item.equipment_id : ""}
    required
    // validators={["required"]}
  ...
  <Autocomplete
    ...
    getOptionLabel={(option, index) => option?.equipment ? option?.equipment : item.item_name}
    filterOptions={(options, params) => {
      const filtered = filter(options, params);
      if (params?.inputValue !== " ") {
        filtered.unshift({
          inputValue: params?.inputValue,
          name: (<Button variant="outlined" color="primary" size="small" value="false" onClick={(event, newValue) => setequipmentdescription(event, index, false)}>+Add Neddw</Button>)
        });
      }
    ...
    onKeyDown={(e) => { controlKeyPress(e, index + 'equipment_id', index + 'description', null, invoiceItemList,true) }}
    onChange={(event, newValue) => setequipmentdescription(event, index, newValue)}
    size="small"
  ...
  localStorage.getItem('division') == 3 ? <></> : <>
    <TableCell className="pl-0 capitalize" align="left" style={{ width: '150px' }}>
      <TextField
        ...
        onKeyDown={(e) => { controlKeyPress(e, index + 'description', index + 'descriptionss', index + 'equipment_id', invoiceItemList,true) }}
...
{item.equipment_price_list.map((e, key) => (
  ...
  {<><FormControl variant="outlined" className={classes.formControl}>
    ...
    >
      ...
      {item?.equipment_price_list?.map((item, id) => (
  ...
  </FormControl>{item.equipment_id ? (<Tooltip title="add price" fontSize="1.2em"><Icon onClick={() => setequipmentids(item.equipment_id, index)}>add</Icon></Tooltip>) : ''}</>}
...
<Icon aria-label="expand row" size="small" style={{width:'25%',float:'left',cursor:'pointer'}} onClick={() => {
  setMargin(item.equipment_id,index,item.name);
}}>
...
<div style={{display:'inline-block',marginLeft:100}}><TextValidator
            ...
            <div className="px-4 flex justify-between">
              <div className="flex">
                ...
                <div className="pr-12">
                  ...
                  <p className="mb-6" style={{ position: 'relative', top: '10px' }}>1.Single shift (260hrs.):</p>
                  <p className="mb-8" style={{ position: 'relative', top: '17px' }}>2.Terms:</p>
                  <p className="mb-10" style={{ position: 'relative', top: '18px' }}>3.Terms:</p>
                  <p className="mb-10" style={{ position: 'relative', top: '15px' }}>4. Payment Terms:</p>
                  <p className="mb-8" style={{ position: 'relative', top: '10px' }}>5. Duration of Hire:</p>
                  <p className="mb-8" style={{ position: 'relative', top: '10px' }}>6.Terms:</p>
                  <p className="mb-8" style={{ position: 'relative', top: '10px' }}>7. Quotation Validity:</p>
                  <p className="mb-8" style={{ position: 'relative', top: '10px' }}>8.Terms:</p>
                  <p className="mb-8" style={{ position: 'relative', top: '10px' }}>9.Terms:</p>
                  <p className="mb-8" style={{ position: 'relative', top: '10px' }}>10.Terms:</p>
                  <p className="mb-8" style={{ position: 'relative', top: '10px' }}>11.Terms:</p>

                </div>
                <div>
                  <TextValidator
                    label="Single shift (260hrs.)"
                    className="mb-4"
                    type="text"
                    variant="outlined"
                    size="small"
                  ...
                  />
                  <TextValidator
                    label="Duration of Hire"
                    onChange={e => setterms5(e.target.value)
                    }
                    type="text"
                    className="mb-4"
                    variant="outlined"
                  ...
                  <TextValidator
                    label="Terms 6"
                    onChange={e => setterms6(e.target.value)
                    }
                    type="text"
                    className="mb-4"
                    variant="outlined"
                  ...
                  <TextValidator
                    ...
                    onChange={e => setterms7(e.target.value)
                    }
                    type="text"
                    className="mb-4"
                    variant="outlined"
                  ...
                  <TextValidator
                    label="Terms 8"
                    onChange={e => setterms8(e.target.value)
                    }
                    type="text"
                    className="mb-4"
                    variant="outlined"
                  ...
                  <TextValidator
                    label="Terms 9"
                    onChange={e => setterms9(e.target.value)
                    }
                    type="text"
                    className="mb-4"
                    variant="outlined"
                  ...
                  <TextValidator
                    label="Terms 10"
                    onChange={e => setterms10(e.target.value)
                    }
                    type="text"
                    className="mb-4"
                    variant="outlined"
                  ...
                  <TextValidator
                    label="Terms 11"
                    onChange={e => setterms11(e.target.value)
                    }
                    type="text"
                    className="mb-4"
                    variant="outlined"
        ...
        )} */}
        {
          shouldOpenEditorDialog && (
            <MemberEditorDialog
              ...
              open={shouldOpenEditorDialog}
              catid={catid}
              equipmentprice={setequipmentprice}
            />
          )
        }
        {
        ...
        {
          shouldOpenEditorDialogequipment && (
            <MemberEquipmentDialog
              handleClose={handleDialogClose}
              open={shouldOpenEditorDialogequipment}
        ...
        }
        {
          shouldOpenConfirmationDialogequipment && (
            <ConfirmationDialog
              open={shouldOpenConfirmationDialogequipment}
              onConfirmDialogClose={handleDialogClose}
