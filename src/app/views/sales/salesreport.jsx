...
const Master = ({
  ...
  useEffect(() => {
    ...
    document.title = "SALES VAT REPORT";
    url.get('salesTax').then(({ data }) => {
      ...
      var result22 = data?.sort(function (a, b) {
      ...
      var sumTotal = result22.reduce((initial, cal) => initial = initial + parseFloat(cal.grand_total), 0)
      settotal(sumTotal)
      var sumVat = result22.reduce((initial, cal) => initial = initial + parseFloat(cal.vat_in_value), 0)
      setvattotal(sumVat)
      var subTotal = result22.reduce((initial, cal) => initial = initial + parseFloat(cal.total_value), 0)
  ...
  const filter_data = (fDate, tDate) => {
    ...
    var result = response_data.filter(obj => (moment(obj.issue_date).format('YYYY-MM-DD') >= moment(fDate).format('YYYY-MM-DD') && moment(obj.issue_date).format('YYYY-MM-DD') <= moment(tDate).format('YYYY-MM-DD') && parseInt(obj.div_id) == parseInt(localStorage.getItem('division')) && parseInt(obj.exclude_from_vat) == 0));
    ...
    setvattotal(sumVat)
    var subTotal = result.reduce((initial, cal) => initial = initial + parseFloat(cal.total_value), 0)
    setsubtotal(subTotal)
  ...
  const handleXlsx = () => {
    ...
    XLSX.writeFile(wb, `AMACO SALES-VAT REPORT ${moment(from_date).format('DD-MM-YYYY')} - ${moment(to_date).format('DD-MM-YYYY')}.xlsx`);
  }
                      ...
                      <div className="px-4 mb-2 pl-4 pt-4 flex ">
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              <h4> SALES VAT </h4>
                              <h6>FROM {moment(from_date).format('DD MMM YYYY')}&nbsp; - &nbsp;{moment(to_date).format('DD MMM YYYY')}</h6>
                            </TableCell>
