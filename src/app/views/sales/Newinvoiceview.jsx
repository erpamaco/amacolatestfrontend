...
const SimpleMuiTable = () => {
  ...
  const [from_date_po, setfrom_date_po] = useState(moment(new Date()).format("MMM YYYY"));
  ...
  const handleDateChange_Q = (date) => {
    if(localStorage.getItem("dataKeyState_po")){
      setfrom_date_po(moment(date).format("MMM YYYY"));
      localStorage.setItem("from_date_po",moment(date).format("MMM YYYY"));
      ...
      setpodetails(datas);
      localStorage.setItem('dataKeyState_po', JSON.stringify(datas))
  ...
  const handleDateChange_Q_Year = (date) => {
    if(localStorage.getItem("dataKeyState_po")){
      setfrom_date_po(moment(date).format("YYYY"));
      localStorage.setItem("from_date_po",moment(date).format("YYYY"));
      setpodetails(resetuserList)
      const datas = resetuserList.filter(
        (obj) =>
  ...
  const handleReset = () => {
    setfrom_date_po(new Date())
    // console.log("sup",from_date_q)
    
  
    localStorage.removeItem("from_date_po");
      
    localStorage.removeItem("dataKeyState_po")
    setpodetails(
  ...
  const handleSubmit = () => {
    const datas = resetuserList.filter(
  (obj) =>
    new Date(moment(obj.ps_date).format('YYYY')).getTime() == new Date(from_date_po).getTime() 
    // new Date(obj.issue_date).getTime() <= new Date(to_date).getTime()
);
setpodetails(datas);
localStorage.setItem('dataKeyState_po', JSON.stringify(datas))
...
// console.log("dataKeyState",JSON.parse(localStorage.getItem("dataKeyState")))
  ...
  if(localStorage.getItem("from_date_po")){
    setfrom_date_po(localStorage.getItem("from_date_po"));
    setLoad(false);
    ...
    url.get("purchase-invoice-list").then(({ data }) => {
      ...
      if(localStorage.getItem("dataKeyState_po")){
        setpodetails(JSON.parse(localStorage.getItem("dataKeyState_po")));
        setLoad(false);
      }else{
      if (data.length) {
  ...
  return (
    ...
    <KeyboardDatePicker
                      ...
                      autoOk={true}
                      views={["month","year"]}
                      value={localStorage.getItem("from_date_po") ? localStorage.getItem("from_date_po") : from_date_po}
                      format="MMM yyyy"
    ...
    <KeyboardDatePicker
          ...
          {/* <Button
                  ...
                  {localStorage.getItem("dataKeyState_po") &&<Button
                    color="error"
                    variant="outlined"
