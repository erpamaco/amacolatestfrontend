...
const PoTrash = ({tabIndex}) => {
  ...
  const [from_date_pot, setfrom_date_pot] = useState(moment(new Date()).format("MMM YYYY"));
  ...
  const handleReset = () => {
    setfrom_date_pot(new Date())
    // console.log("sup",from_date_q)
    
  
    localStorage.removeItem("from_date_pot");
      
    localStorage.removeItem("dataKeyState_pot")
    setpodetails(
  ...
  (obj) =>
    new Date(moment(obj.created_at).format('YYYY')).getTime() == new Date(from_date_pot).getTime() 
    // new Date(obj.issue_date).getTime() <= new Date(to_date).getTime()
);
setpodetails(datas);
localStorage.setItem('dataKeyState_pot', JSON.stringify(podetails))
...
const handleDateChange_Q_Year = (date) => {
  if(localStorage.getItem("dataKeyState_pot")){
    setfrom_date_pot(moment(date).format("YYYY"));
    localStorage.setItem("from_date_pot",moment(date).format("YYYY"));
...
const handleDateChange_Q = (date) => {
  if(localStorage.getItem("dataKeyState_pot")){
    setfrom_date_pot(moment(date).format("MMM YYYY"));
    localStorage.setItem("from_date_pot",moment(date).format("MMM YYYY"));
    setpodetails(resetuserList)
    const datas = resetuserList.filter(
      (obj) =>
  ...
  if(localStorage.getItem("from_date_pot")){
    setfrom_date_pot(localStorage.getItem("from_date_pot"));
    setLoad(false);
  }
    url.get("purchase-invoice-list").then(({ data }) => {
      ...
      if(localStorage.getItem("dataKeyState_pot")){
        setpodetails(JSON.parse(localStorage.getItem("dataKeyState_pot")));
        setLoad(false);
      }else{
      if (data.length) {
  ...
  return (
    ...
    <KeyboardDatePicker
                      ...
                      autoOk={true}
                      views={["month","year"]}
                      value={localStorage.getItem("from_date_pot") ? localStorage.getItem("from_date_pot") : from_date_pot}
                      format="MMM yyyy"
    ...
    <KeyboardDatePicker
          ...
          {/* <Button
                  ...
                  {localStorage.getItem("dataKeyState_pot") &&<Button
                    color="error"
                    variant="outlined"
