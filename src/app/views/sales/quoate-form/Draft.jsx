...
const SimpleMuiTable = () => {
  ...
  const [resetuserList, setResetUserList] = useState([]);
   const [from_date_q_d, setfrom_date_q_d] = useState(moment(new Date()).format("MMM YYYY"));
  ...
  const handleDateChange_Q_Year = (date) => {
    if(localStorage.getItem("dataKeyState_q_d")){
      setfrom_date_q_d(moment(date).format("YYYY"));
      localStorage.setItem("from_date_q_d",moment(date).format("YYYY"));
      setqdetails(resetuserList)
      const datas = resetuserList.filter(
        (obj) =>
          new Date(moment(obj.quote_date).format('YYYY')).getTime() == new Date(moment(date).format("YYYY")).getTime() 
          // new Date(obj.issue_date).getTime() <= new Date(to_date).getTime()
      );
      setqdetails(datas);
      localStorage.setItem('dataKeyState_q_d', JSON.stringify(datas))
   ...
   const handleDateChange_Q = (date) => {
    if(localStorage.getItem("dataKeyState_q_d")){
      setfrom_date_q_d(moment(date).format("MMM YYYY"));
      localStorage.setItem("from_date_q_d",moment(date).format("MMM YYYY"));
      setqdetails(resetuserList)
      const datas = resetuserList.filter(
        (obj) =>
          new Date(moment(obj.quote_date).format('MMM YYYY')).getTime() == new Date(moment(date).format("MMM YYYY")).getTime() 
          // new Date(obj.issue_date).getTime() <= new Date(to_date).getTime()
      );
      setqdetails(datas);
      localStorage.setItem('dataKeyState_q_d', JSON.stringify(datas))
  ...
  const columnStyleWithWidth = {
    ...
    const handleReset = () => {
    setfrom_date_q_d(new Date())
    
  
    localStorage.removeItem("from_date_q_d");
      
    localStorage.removeItem("dataKeyState_q_d")
    setqdetails(
      resetuserList.filter(
        (obj) =>
          new Date(moment(obj.quote_date).format('YYYY')).getTime() == new Date(curr_year).getTime() 
          // new Date(obj.issue_date).getTime() <= new Date(to_date).getTime()
      )
    );
    setIsAlive(false)
  ...
  const handleSubmit = () => {
    const datas = resetuserList.filter(
  (obj) =>
    new Date(moment(obj.quote_date).format('MMM YYYY')).getTime() == new Date(from_date_q_d).getTime() 
    // new Date(obj.issue_date).getTime() <= new Date(to_date).getTime()
);
setqdetails(datas);
localStorage.setItem('dataKeyState_q_d', JSON.stringify(datas))
...
// console.log("dataKeyState",JSON.parse(localStorage.getItem("dataKeyState")))
  ...
  if(localStorage.getItem("from_date_q_d")){
    setfrom_date_q_d(localStorage.getItem("from_date_q_d"));
    ...
    url.get("sales-list").then(({ data }) => {
      ...
      // {
        if(localStorage.getItem("dataKeyState_q_d")){
          setqdetails(JSON.parse(localStorage.getItem("dataKeyState_q_d")));
          setLoad(false);
        }else{
  ...
  return (
    ...
    <KeyboardDatePicker
                      ...
                      autoOk={true}
                      views={["month","year"]}
                      value={localStorage.getItem("from_date_q_d") ? localStorage.getItem("from_date_q_d") : from_date_q_d}
                      format="MMM yyyy"
    ...
    <KeyboardDatePicker
          ...
          {/* <Button
                  ...
                  {localStorage.getItem("dataKeyState_q_d") &&<Button
                    color="error"
                    variant="outlined"
