...
const RejectQuote = () => {
  ...
  const [from_date_q_t, setfrom_date_q_t] = useState(moment(new Date()).format("MMM YYYY"));
  ...
  if(localStorage.getItem("from_date_q_t")){
    setfrom_date_q_t(localStorage.getItem("from_date_q_t"));
    ...
    url.get("quotations-rejected-list").then(({ data }) => {
      ...
      // if(data.length)
      // {
        if(localStorage.getItem("dataKeyState_q_t")){
          setqdetails(JSON.parse(localStorage.getItem("dataKeyState_q_t")));
          setLoad(false);
  ...
  (obj) =>
    new Date(moment(obj.ps_date).format('MMM YYYY')).getTime() == new Date(from_date_q_t).getTime() 
    // new Date(obj.issue_date).getTime() <= new Date(to_date).getTime()
);
setqdetails(datas);
localStorage.setItem('dataKeyState_q_t', JSON.stringify(datas))

// localStorage.setItem('dataKeyState', userList)
// console.log("dataKeyState",JSON.parse(localStorage.getItem("dataKeyState")))
};
  const handleReset = () => {
    setfrom_date_q_t(new Date())
    
  
    localStorage.removeItem("from_date_q_t");
      
    localStorage.removeItem("dataKeyState_q_t")
    setqdetails(
  ...
  const handleDateChange_Q_Year = (date) => {
    if(localStorage.getItem("dataKeyState_q_t")){
      setfrom_date_q_t(moment(date).format("YYYY"));
      localStorage.setItem("from_date_q_t",moment(date).format("YYYY"));
  ...
  const routerHistory = useHistory();
    ...
    if(localStorage.getItem("dataKeyState_q_t")){
      setfrom_date_q_t(moment(date).format("MMM YYYY"));
      localStorage.setItem("from_date_q_t",moment(date).format("MMM YYYY"));
      setqdetails(resetuserList)
      const datas = resetuserList.filter(
        (obj) =>
  ...
  return (
    ...
    <KeyboardDatePicker
                      ...
                      autoOk={true}
                      views={["month","year"]}
                      value={localStorage.getItem("from_date_q_t") ? localStorage.getItem("from_date_q_t") : from_date_q_t}
                      format="MMM yyyy"
    ...
    <KeyboardDatePicker
          ...
          {/* <Button
                  ...
                  {localStorage.getItem("dataKeyState_q_t") &&<Button
                    color="error"
                    variant="outlined"
