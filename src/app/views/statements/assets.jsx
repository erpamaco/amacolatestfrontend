...
//   bank statements
  ...
  useEffect(() => {
    expensePaid().then(({ data }) => {
        ...
        setarr(data?.filter(obj=>obj.account_categories[0].parent_id==48))
        setbalance(data?.filter(obj=>obj.account_categories[0].parent_id==48).reduce((initial,cal)=> initial= initial+ parseFloat(cal.amount),0))
        setresponse(data?.filter(obj=>obj.account_categories[0].parent_id==48))
    })
  }, []);
...
//   function to set the To date and call the filter_data function
  ...
  return (
    <div className="m-sm-30">
      ...
      <div className={clsx("invoice-viewer py-4 pt-0", classes.Assets)}>
